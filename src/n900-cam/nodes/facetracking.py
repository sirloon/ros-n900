...
class face_track(object):
  ...
  def __init__(self):
    ...
    self.image_sub = rospy.Subscriber("/gscam/image_rect_color",Image,self.callback)
    self.cascade = cv.Load("data/haarcascade_frontalface_alt.xml")
    #self.cascade = cv.Load("data/haarcascade_profileface.xml")
  ...
  def track(self,img,init_rect):
      ...
      # Run the cam-shift
      cv.CalcBackProject( [self.sat], backproject, self.hist )
      ...
      print "\titers: %s, aera: %s, value: %s, rect: %s" % (iters,area,value,rect)
      self.track_window = rect
      print "new track_window: %s" % repr(self.track_window)

      cv.CalcArrHist( [self.sat], self.hist, 0)
      ...
      if not is_rect_nonzero(self.track_window):
          ...
          self.tracking = False
      return img, self.hue_histogram_as_image(self.hist)
  ...
  def callback(self,data):
    ...
    if self.rects and self.tracking:
        ...
        cv_trackimg,his = self.track(cv_image,self.rects[0])
        cv.WaitKey(3)
    else:
